...
global ALPHA       # The fine structure constant α is 1/137.036...
ALPHA = 137.036    # Values are set to this to indicate that an error occurred.


def set_Tk_var():
    global focal_select
    focal_select = tk.IntVar()
    global object_select
    object_select = tk.IntVar()
    global image_select
    image_select = tk.IntVar()
    global magnification_select
    magnification_select = tk.IntVar()
    global focal_text
    focal_text = tk.StringVar()
    global object_text
    object_text = tk.StringVar()
    global image_text
    image_text = tk.StringVar()
    global magnification_text
    magnification_text = tk.StringVar()
    global radio
    radio = tk.StringVar()
    global message
    message = tk.StringVar()
    global sig_figs
    sig_figs = tk.StringVar()
...
def UserTyped(event):
    ...
    if key_in in "0123456789+-.e:" and key_in != "":
        ...
        if key_in in string.digits:
            Calculate()


def KeyPressed(event):
    """Handle Enter, BackSpace, and Delete keys." are directed here."""
    sym = event.keysym
    if event.widget.selection_present():
...
def GenerateList():
    ...
    if len(range_bits) == 3:
        ...
        step = 1.0
    num_entries = int((end - start) / step)
    if num_entries > 1000:
        warn =("About " + str(num_entries) + " entries will be generated.\n" +
               "Continue?")
        if messagebox.askyesno("Continue?", warn) is False:
            return
...
def ShowHelp():
    helpfile = os.getcwd()+"\\LPD Help.html"
    cmd = "start " + helpfile
    os.system(cmd)
